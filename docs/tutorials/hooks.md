...
This tutorial is designed to showcase what we consider best practices for implementing Redux in a React project. We've tried to keep these explanations beginner-friendly, but we do need to make some assumptions about what you already know:
...
__Those steps provide the basic outline for this tutorial.__
## Project Overview
...
- Users should be able to toggle a Todo item between a `Completed` and `Not Completed` state
...
If you want to follow along with the tutorial, please fork our [Stater Template](./needs-link.md) on CodeSandbox. A completed code base is available for reference at the end of the tutorial.
...
The first step in any Redux project is creating your [Store](https://redux.js.org/api/store). The Redux Store is a JavaScript object that that holds our applications [State](https://redux.js.org/understanding/thinking-in-redux/glossary#state).

To begin, in the `/src` directory of your project, create a `store.js` file. This is where we will define the entry point to our application's store. We can instantiate our store using the `configureStore()` helper from Redux Toolkit. It's a helpful abstraction that allows us to quickly setup a Redux store while also providing improved developer experience versus the default `createStore()` method.

`configureStore()` accepts a configuration object with many different options to help define our Redux store. The first (and most important for our purposes) is the `reducer` field, which accepts either a single root reducer or multiple entries to be automatically combined. We are going to break our store into domain specific units that we call `slices` and will add them as fields here, where, once combined, they will represent our global state tree. For now though, let's just simply lay the skeleton for our future code:
...
Thinking about each part of global state as an individual __"slice"__ is a great way to make state management more, well… manageable.

Each slice should correspond to a feature in your application and will hold its state, its action creators for dispatching updates, as well as any [selectors](https://redux.js.org/tutorials/fundamentals/part-2-concepts-data-flow#selectors) that can be used at the component level of our application to reference that state.

Again we will use a Redux Toolkit method `createSlice()` to implement our slice. `createSlice()` receives an object with three main configuration fields:
...
- `reducers`: a Map-like object that contains key/value pairs made up of an _action type_ string and its corresponding _case reducer_ function for predictably updating state.

With this definition in mind, let's define our first slice of global state!

**Todo slice**

If you forked the [example project](/needs-link), you will see that we already have some pre-created folders inside `src`. The `feature` directory is _especially_ important, because it's where most of our code will live. Each feature will have its own corresponding subfolder, which will contain, amongst other things, that feature's corresponding slice of state.

:::important A not on file structure
The "feature folder" / "slice file" is a very common pattern for Redux applications, and it's the structure that we recommend. For more information about this and other patterns, check out this [FAQ answer](https://redux.js.org/faq/code-structure) on the official Redux documentation.
:::
...
1. Our slice's `initialState`. We keep this in a variable instead of directly passing it to `createSlice` because it's sometimes useful to be able reference it in our reducers. For example, to reset some field to its initial value. And remember, this only represents the initial state for **this** slice, not for the whole state tree.
2. The actual slice, generated with the help of RTK's [createSlice()](https://redux-toolkit.js.org/usage/usage-guide#simplifying-slices-with-createslice) method. This will host both our actions and reducers, ready for usage in our React code.
...
Ok, now that we have our first slice, it's time to wire it up to our store. In `store.js` we start by importing our `todosReducer`, which we just created in `todosSlice.js`. Then, we pass that to the `reducer` field in `configureStore`. The functions passed to `reducer` make up our root reducer, which will be ultimately responsible for holding our redux state and acting as its single source of truth.
...
In this case, dispatching that action will push a new Todo to our slice's state. The new item will be assigned an id number based on incrementing the previous `nextTodoId` value, set its content using the value from the action object `payload`, and have its `isCompleted` flag set to `false`.

You may notice that in defining how our reducer updates state, we appear to be pushing our new value directly onto our `initialState` array. If you have previous experience with Redux, that might seem very odd, since one of the most important principles of Redux is that [its state should always be updated immutably](https://redux.js.org/understanding/thinking-in-redux/three-principles#changes-are-made-with-pure-functions).

While appearing to contradict that rule, our logic is leveraging another strength of Redux Toolkit. RTK ships with the [Immer](https://immerjs.github.io/immer/) library, which enables us to write mutable logic to express our desired changes and then automatically applies the difference from the previous state as an immutable update.
...
One of the benefits of using `createSlice` is that it automatically generates a corresponding action creator for each of our reducer cases. Here we are choosing to destructure the individual function and export it as a separate value to be able to reference it more succinctly at the component level.

:::tip
For more on how the `reducers` field works as a "builder callback" to add reducers to the slice, check out [the official RTK docs](https://redux-toolkit.js.org/api/createSlice#parameters). 
:::
### Testing our Reducer

Before we dispatch our action from the UI layer, let's check to see that everything in our slice is working correctly. We can call our `todoAdded` action from `index.js` and confirm that everything is working as expected with [Redux Dev Tools](https://redux-toolkit.js.org/usage/usage-guide#simplifying-store-setup-with-configurestore).
...
We are confident that our reducer slice is updating the store as expected. Now let's jump to our `<AddTodo>` component and explore how we can transmit a `todoAdded` action back to our `todoSlice`. 
...
Now, when we click the `Add Todo` button our `handleSubmit` function calls the `useDispatch` hook, instantiated here as just `dispatch`. Dispatch takes one argument, the `todoAdded` action, which passes the value from `input`, its payload.
